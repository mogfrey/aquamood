...
const api_url= 'http://geoffish.herokuapp.com/api'
...
var pages={
    ...
    temperature: function(){
        ...
        window.location.href='../pages/temp.html';

    },
    distance:function(){

        window.location.href='../pages/distance.html';

    },
    ph:function(){

        window.location.href='../pages/ph.html';
...
var statistics ={
    getStats: function() {
        ...
        clear_data:function (argument) {
            // body...
            $( "#loading_screen_div" ).show(); 
            var settings = {
              "async": true,
              "crossDomain": true,
              "url": api_url+"/clear_data/",
              "method": "GET",
              "headers": {
            ...
            $.ajax(settings).done(function (response) {
                ...
                if (response.status_code==200){
                    ...
                    Materialize.toast('Success Data Cleared!', 4000);

                }else{


                    Materialize.toast('Error Clearing Data!', 4000);
                }
            });
    ...
    register:function (argument) {
      ...
      $.ajax(settings).done(function (response) {
        ...
        if (response.status_code==500) {
            ...
            Materialize.toast('Unknown Error Occurred Try Again Later', 4000);
        }
        ...
        $.ajax(settings).done(function (response) {
          ...
          for (var i = response.data.length - 1; i >= 0; i--) {
            ...
            var data = '<li class="collection-item avatar" style="margin:5px;" onclick="chat.load_chat('+response.data[i].id+');">'+
                        ...
                        '</li>';
            if (response.data[i].id == authentication.data().id) {continue;}            
